...
import WifiCredentials from "../components/drive/WifiCredentials";
import { wifiPasswordDisplay, wifiSsidDisplay } from "../components/drive/wifi";
...
export default function DriveFlowPage() {
  ...
  const goToRailboxLocoList = async () => {
    if (!user) return;
    setBusy(true);
    setError(null);
    try {
  ...
  const onUserPicked = (picked: User) => {
    ...
    if (device && isRailboxDevice(device)) {
      setBusy(false);
      setPhase("railboxAppQr");
      return;
    }
    if (device && isWithrottleAdvancedDevice(device)) {
      setBusy(false);
      setPhase("wtWifi");
      return;
    }
  ...
  return (
    <AppShell title={t("drive.heading")} showBack>
      ...
      <Paper sx={{ p: { xs: 2, sm: 4 } }}>
        ...
        {phase === "railboxLocoList" && (
          <RailboxLocoListStep vehicles={vehicles} onDone={() => navigate("/")} />
        )}
            ...
            <Stack direction="row" justifyContent="space-between" sx={{ mt: 3 }}>
              <Button
                ...
                onClick={() => {
                  ...
                  else if (isWithrottleAdvancedDevice(device)) setPhase("wtWifi");
                  else void cancel();
                }}
              >
                {t("app.back")}
        ...
        {phase === "pairing" && pairing && device && (
          <PairingStep
            ...
            onContinue={() => {
              if (isWlanmausDevice(device)) setPhase("wlanmausDriveMode");
              else if (isRailboxDevice(device)) void goToRailboxLocoList();
              else void goPickLoco();
            }}
            onRetry={() => {
        ...
        {phase === "pickLoco" && device && !isWlanmausDevice(device) && !isRailboxDevice(device) && (
          <PickLocoStep
            vehicles={vehicles}
            selected={pickedLoco}
...
function RailboxPairingSetupStep({
  onBack,
  onContinue,
  stationsLoading,
  stationsError,
...
function RailboxLocoListStep({
  ...
  return (
    <Box>
      <Typography variant="h5" sx={{ mb: 1 }}>
        {t("drive.railbox.locoListTitle")}
      </Typography>
      <Typography color="text.secondary" sx={{ mb: 2 }}>
        {t("drive.railbox.locoListHint")}
      </Typography>
      {vehicles === null ? (
        <CircularProgress />
      ) : vehicles.length === 0 ? (
